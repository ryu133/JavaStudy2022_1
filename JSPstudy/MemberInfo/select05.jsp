...
<!-- 
	Statement : 
		...
		- 쿼리에 인자를 부여할 수 없다. 변수를 쿼리에 적용할 때 따옴표 처리를 잘 해주어야 한다
		- 매번 컴파일을 수행해야 됩니다. (cache를 사용하지 않는다)
	
	PreparedStatement : 
		- 쿼리에 인자를 부여할 수 있다. (?)    ? 인자에 변수를 할당한다.
		- 처음 컴파일 된 이후, 컴파일을 수행하지 않음. (cache를 사용한다)
		- 여러번 수행할 때 빠른 속도를 가집니다.
	...
	<table width="500" border="1">
		...
		<%
			...
			try{
				...
				pstmt =conn.prepareStatement(sql); //connection객체의 prepareStatement()로 pstmt를 활성화
				rs=pstmt.executeQuery(sql);
					//pstmt.executeQuery(sql) : select 한 결과를 ResultSet 객체에 저장해야 함.
